...
class ImmichAPI:
    ...
    def __init__(self, config: dict):
        ...
        if not self.api_url:
            self.enabled = False
            logger.warning("Immich API URL未配置，Immich API功能将被禁用")
        elif not self.api_key:
            self.enabled = False
            logger.warning("Immich API密钥未配置，Immich API功能将被禁用")
        else:
            self.enabled = True
            # 初始化 SDK 配置
            self._init_sdk()
            logger.info(f"Immich API客户端已初始化: API={self.api_url}")
    ...
    async def get_asset(self, asset_id: str) -> Optional[Dict]:
        ...
        """
        if not self.enabled:
            logger.warning("Immich API未启用，无法获取资产信息")
            return None
        
        if not asset_id:
            logger.warning("资产ID为空，无法获取资产信息")
            return None
        ...
        try:
            ...
            if asset_dto:
                ...
                logger.debug(f"成功获取资产详情: {asset_id}")
                return result
            
            logger.warning(f"获取资产详情返回空结果: {asset_id}")
            return None
            ...
            logger.error(f"获取资产详情异常: {e}", exc_info=True)
            return None
    ...
    async def search_person(
        ...
        """
        if not self.enabled:
            logger.warning("Immich API未启用，无法搜索人物")
            return None
        
        if not name:
            logger.warning("人物名称为空，无法搜索人物")
            return None
        ...
        try:
            ...
            if persons:
                ...
                if return_ids:
                    ...
                    if person_ids:
                        ...
                        logger.warning(f"未找到有效的人物ID: name='{name}'")
                        return None
                else:
                    logger.info(f"成功搜索人物: name='{name}', 找到 {len(result)} 个人物")
                    return result
            
            logger.warning(f"搜索人物返回空结果: name='{name}'")
            return None
            
        except asyncio.TimeoutError:
            logger.warning(f"搜索人物超时（{timeout}秒）: name='{name}'")
            return None
        except ApiException as e:
            logger.error(
            ...
            logger.error(f"搜索人物异常: {e}, name='{name}'", exc_info=True)
            return None
    ...
    async def search_smart(
        ...
        **kwargs
        ...
        """
        if not self.enabled:
            logger.warning("Immich API未启用，无法执行搜索")
            return None
        
        if not query:
            logger.warning("搜索查询字符串为空，无法执行搜索")
            return None
        ...
        try:
            ...
            logger.info(f"[immich_api] 调用 SDK search_api.search_smart 开始...")
            ...
            logger.info(f"[immich_api] SDK search_api.search_smart 完成，耗时: {api_duration:.2f}秒")
            ...
            logger.warning(f"智能搜索返回空结果: query='{query}'")
            return None
            ...
            logger.error(f"智能搜索异常: {e}", exc_info=True)
            return None
    ...
    async def search_random(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"[immich_api] 调用 SDK search_api.search_random 开始...")
            api_start_time = time.time()
            ...
            api_duration = time.time() - api_start_time
            logger.info(f"[immich_api] SDK search_api.search_random 完成，耗时: {api_duration:.2f}秒")
            ...
            logger.error(f"随机搜索异常: {e}", exc_info=True)
            return None
    ...
    async def download_asset(
        ...
        """
        if not self.enabled:
            logger.warning("Immich API未启用，无法下载资产")
            return None
        
        if not asset_id:
            logger.warning("资产ID为空，无法下载资产")
            return None
        ...
        try:
            ...
            if asset_data:
                logger.info(f"成功下载资产原始文件: {asset_id}, 大小: {len(asset_data)} 字节")
                return bytes(asset_data)
            
            logger.warning(f"下载资产返回空数据: {asset_id}")
            return None
            ...
            logger.error(f"下载资产异常: {e}", exc_info=True)
            return None
    ...
    async def view_asset(
        ...
        """
        if not self.enabled:
            logger.warning("Immich API未启用，无法查看资产")
            return None
        
        if not asset_id:
            logger.warning("资产ID为空，无法查看资产")
            return None
        ...
        try:
            ...
            logger.warning(f"下载资产缩略图返回空数据: {asset_id}")
            return None
            ...
            logger.error(f"下载资产缩略图异常: {e}", exc_info=True)
            return None
    ...
    async def upload_asset(
        ...
        **kwargs
        ...
        """
        if not self.enabled:
            logger.warning("Immich API未启用，无法上传资产")
            return None
        ...
        try:
            ...
            if isinstance(file_path, str):
                ...
                logger.error("file_path必须是文件路径（str）或文件数据（bytes）")
                return None
            ...
            if e.status == 409:
                logger.warning("资产已存在（可能是重复上传）")
            return None
        except FileNotFoundError as e:
            logger.error(f"文件不存在: {e}")
            return None
        except Exception as e:
            logger.error(f"上传资产异常: {e}", exc_info=True)
            return None
