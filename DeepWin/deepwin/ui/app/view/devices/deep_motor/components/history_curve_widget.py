...
class HistoryCurveWidget(QWidget):
    """历史曲线组件"""
    ...
    def setup_ui(self):
        ...
        # 创建通用绘图组件
        self.plot_widget = UniversalPlotWidget(self.title, self.logger, self)
    ...
    def _on_param_changed(self, param_name: str):
        ...
        if self.logger:
            self.logger.info(f"历史曲线组件参数改变: {param_name}")
    ...
    def update_history_data(self, history_data_dict: Dict[str, Any]):
        ...
        """
        if self.logger:
            self.logger.debug(f"历史曲线组件收到数据更新: {self.current_selected_param}")
    ...
    def update_trajectory_data(self, trajectory_data: Dict[str, Any]):
        ...
        """
        if self.logger:
            self.logger.debug(f"历史曲线组件收到轨迹数据更新")
    ...
    def update_execution_progress(self, progress_data: Dict[str, Any]):
        ...
        """
        if self.logger:
            self.logger.debug(f"历史曲线组件收到执行进度数据")
    ...
    def update_teaching_trajectory(self, times: List[float], positions: List[float]):
        ...
        """
        if self.logger:
            self.logger.debug(f"历史曲线组件收到示教轨迹数据，点数: {len(times) if times else 0}")
    ...
    def update_teaching_trajectory(self, times: list, positions: list):
        ...
        """
        if self.logger:
            self.logger.debug(f"历史曲线组件: 收到示教轨迹更新，点数: {len(times) if times else 0}")
    ...
    def update_execution_trajectory(self, progress_data: dict):
        ...
        """
        if self.logger:
            self.logger.debug(f"历史曲线组件: 收到执行轨迹更新")
        self.plot_widget.update_execution_trajectory(progress_data)
    ...
    def set_execution_mode(self, enabled: bool):
        ...
        """
        if self.logger:
            self.logger.debug(f"历史曲线组件: 设置执行模式: {enabled}")
