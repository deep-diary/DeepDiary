...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QGridLayout, QDoubleSpinBox, QSlider
...
class DeepMotorPage(BaseDevicePage):
    ...
    def __init__(self, device_name: str = "DeepMotor", log_manager: LogManager = None, 
        ...
        self.use_planned_trajectory = False
        self.planned_line = None
    ...
    def setup_ui(self):
        ...
        # --- 电机控制卡片 ---
        motor_control_card, motor_control_layout = self.create_control_card('电机控制')
        ...
        self.pos_slider.setValue(0)  # 默认值为0
        self.pos_slider.setTickPosition(QSlider.TicksBelow)
        self.pos_slider.setTickInterval(60)
        self.pos_value_label = QLabel('0')
        self.pos_slider.valueChanged.connect(self.on_position_changed)
        
        speed_label = QLabel('速度:')
        self.speed_slider = QSlider(Qt.Horizontal)
        self.speed_slider.setRange(-20, 20)
        self.speed_slider.setValue(5)  # 默认值为5
        ...
        # --- 示教控制卡片 ---
        teaching_card, teaching_layout_v = self.create_control_card('示教控制')
    ...
    def on_set_pos_clicked(self):
        """设置位置按钮点击处理函数"""
        if self.logger:
            self.logger.info(f"设置位置按钮被点击，电机ID: {self.id_spin.value()}, 位置: {self.pos_slider.value()}")
        self.send_command('motor_set_pos', [self.id_spin.value(), self.pos_slider.value()])

    def on_set_pos_speed_clicked(self):
        """设置位置和速度按钮点击处理函数"""
        if self.logger:
            self.logger.info(f"设置位置和速度按钮被点击，电机ID: {self.id_spin.value()}, 位置: {self.pos_slider.value()}, 速度: {self.speed_slider.value()}")
        self.send_command('motor_set_pos_speed', [self.id_spin.value(), self.pos_slider.value(), self.speed_slider.value()])
    ...
    def on_jog_pressed(self):
        """点动按钮按下时的处理函数"""
        if self.logger:
            self.logger.info(f"点动按钮按下，电机ID: {self.current_motor_id}, 速度: {self.speed_slider.value()}")
        self.send_command('motor_jog', [self.current_motor_id, self.speed_slider.value()])
    ...
    def init_trajectory_list(self):
        ...
        if self.logger:
            self.logger.info(f"DeepMotor页面: 准备发射轨迹列表请求信号")
    ...
    def update_trajectory_list(self, trajectory_names: list, prefer_newest: bool = False):
        """更新轨迹列表"""
        if self.logger:
            self.logger.info(f"DeepMotor页面: 收到轨迹列表更新，轨迹数量: {len(trajectory_names)}, 轨迹列表: {trajectory_names}")
        ...
        if prefer_newest and trajectory_names:
            ...
            if self.logger:
                self.logger.info(f"DeepMotor页面: 优先选中最新轨迹 '{newest_trajectory}'")
            ...
            if self.logger:
                self.logger.info(f"DeepMotor页面: 保持选中轨迹 '{current_selection}'")
        else:
            # 如果没有之前选中的轨迹或轨迹不存在，清空选择
            self._current_trajectory = None
            if self.logger:
                self.logger.info("DeepMotor页面: 清空轨迹选择")
        
        # 更新删除按钮状态
        self.delete_trajectory_button.setEnabled(bool(self.trajectory_combo.currentText()))
        
        if self.logger:
            self.logger.info(f"DeepMotor页面: 轨迹列表更新完成，当前选中: {self._current_trajectory}")
    ...
    def on_trajectory_selection_changed(self, text: str):
        ...
        # 2. 直接发送请求，自动刷新曲线
        self.request_trajectory_data.emit(self.device_name, self._current_trajectory)
    ...
    def on_restore_time_clicked(self):
        ...
        self.restore_default_requested.emit(self.device_name, self._current_trajectory)
    ...
    def on_param_changed(self, param_text: str):
        ...
        if self.current_selected_param.startswith('trajectory_'):
            ...
            self.request_history_data.emit(self.device_name, self.current_selected_param)
    ...
    def on_refresh_clicked(self):
        ...
        if self.current_selected_param.startswith('trajectory_') and self._current_trajectory:
            ...
            self.replan_requested.emit(self.device_name, self._current_trajectory, duration)
            # 点击后禁用，等待新数据
            self.refresh_button.setEnabled(False)
        else:
            # 普通模式：发送普通刷新请求
    ...
    def on_start_teaching_clicked(self):
        ...
        # 发送开始示教信号
        self.start_teaching_requested.emit(self.device_name, self.current_motor_id)
    ...
    def on_execute_teaching_clicked(self):
        ...
        self.execute_teaching_requested.emit(self.device_name, trajectory_name, self.planning_switch.isChecked(), self.current_motor_id)
    ...
    def on_trajectory_execution_finished(self):
        if self.logger:
            self.logger.info("DeepMotor页面: 收到轨迹执行完成信号，恢复按钮和状态")
        self.plot_update_timer.setInterval(50)
