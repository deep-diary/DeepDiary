...
from app_logic.device_logic_manager.protocol_processing.base import ProtocolProcessor, ProtocolCommand, ProtocolResponse
from app_logic.device_logic_manager.protocol_processing.devices.utils import calculate_checksum, bytes_to_hex_str
...
class TemplateProtocol(ProtocolProcessor):
    ...
    def __init__(self, config_file=None):
        ...
        super(TemplateProtocol, self).__init__(config_file)
    ...
    def encode_command(self, command: TemplateCommand) -> bytes:
        ...
        try:
            if not isinstance(command, TemplateCommand):
                self.logger.error("命令对象类型错误")
                return None
            
            if command.command_code is None:
                self.logger.error(f"命令 {command.command_type} 没有有效的命令代码")
                return None
            ...
            self.logger.error(f"编码命令失败: {str(e)}")
            return None
    ...
    def decode_response(self, data: bytes) -> TemplateResponse:
        ...
        try:
            if not data or len(data) < 4:  # 最小包长度检查
                self.logger.error("数据包太短")
                return None
            
            # 检查起始和结束字节
            if data[0] != self.start_byte or data[-1] != self.end_byte:
                self.logger.error("数据包格式错误，缺少起始或结束标志")
                return None
            ...
            if checksum_received != checksum_calculated:
                self.logger.error(f"校验和失败: 计算值={checksum_calculated}, 接收值={checksum_received}")
                return None
            ...
            if not response_type:
                self.logger.error(f"未知响应代码: 0x{response_code:02X}")
                return None
