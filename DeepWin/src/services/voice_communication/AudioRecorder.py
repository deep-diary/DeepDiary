...
class AudioRecorder:
    ...
    def __init__(self, pya: pyaudio.PyAudio, sample_rate=16000, chunk_size=AUDIO_CHUNK_SIZE, 
        ...
        with self.status_lock:
            if self.status == 'stopped':
                ...
                if not self.recorder_stream or not self.recorder_stream.is_active():
                    logger.info("音频流无效，尝试重新初始化...")
                    if not self._reinitialize_stream():
                        logger.error("无法初始化音频流，录制启动失败")
                        return False
                ...
                if self.recorder_thread is None or not self.recorder_thread.is_alive():
                    ...
                    logger.debug("音频录制器开始录制")
                else:
                    logger.debug("音频录制器已在录制中")
                
                return True
            else:
                logger.debug("音频录制器已在录制状态")
                return True
    ...
    def stop_recording(self):
        ...
        with self.status_lock:
            if self.status == 'recording':
                self.status = 'stopped'
                self.should_record = False
                logger.debug("音频录制器停止录制")
                return True
            else:
                logger.debug("音频录制器已在停止状态")
                return True
    ...
    def recorder_loop(self):
        ...
        while self.status != 'stopped':
            ...
            try:
                # 检查音频流是否仍然有效
                if not self.recorder_stream or not self.recorder_stream.is_active():
                    logger.warning("音频流已关闭，重新初始化...")
                    self._reinitialize_stream()
                    if not self.recorder_stream:
                        logger.error("无法重新初始化音频流，退出录制循环")
                        break
                    continue
                ...
                if "Stream closed" in str(e) or "Stream not active" in str(e):
                    logger.info("检测到音频流关闭，尝试恢复...")
                    if self._reinitialize_stream():
                        logger.info("音频流恢复成功，继续录制")
                        continue
                    else:
                        logger.error("音频流恢复失败，退出录制循环")
                        break
                else:
                    # 其他错误，短暂等待后继续
                    logger.warning(f"其他错误，等待后重试: {e}")
                    time.sleep(0.1)
                    continue
