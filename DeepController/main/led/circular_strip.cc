...
void CircularStrip::Breathe(StripColor low, StripColor high, int interval_ms) {
    StartStripTask(interval_ms, [this, low, high]() {
        ...
        static float brightness = 0.0f;  // 使用浮点数亮度值 (0.0 - 1.0)
        static const float step = 0.05f;  // 每次变化的步长
        ...
        // 根据亮度值计算当前颜色
        StripColor current_color;
        current_color.red = (uint8_t)(low.red + (high.red - low.red) * brightness);
        current_color.green = (uint8_t)(low.green + (high.green - low.green) * brightness);
        current_color.blue = (uint8_t)(low.blue + (high.blue - low.blue) * brightness);
